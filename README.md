...
3. 给每张图打分（质量 + 美观度，美观度模型可在 UI 里切换），组内排序；
4. 每组把「最优候选」顶到最前，你在 WebUI 里一眼扫、点选确认；
...
打开浏览器 → **http://127.0.0.1:7861** → 填入图片文件夹路径 → Scan。
...
- `config/curator.example.yaml` — 各打分项权重、缩略图尺寸等。复制为 `config/curator.yaml` 生效。
